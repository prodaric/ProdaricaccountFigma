...
* xref:example-business-model.adoc[Modelo de negocio de ejemplo] — Corporación Coderic (multi-tenant: SAS/Colombia, SA/Venezuela, Corporation/USA).
* xref:wireframes.adoc[Wireframes] — Layouts globales y pantallas del sistema en ASCII: entidad/período, plan de cuentas, journal, diario, saldos, reportes, configuración; pantallas ampliadas (dashboard, detalle de asiento, balance de comprobación, cierre de período, administración entidad/período, reversión de asiento, audit log); opcional sostenibilidad.
* xref:architecture.adoc[Arquitectura] — Diagramas Mermaid: flujo de registro, schemas, flujo de lectura.
